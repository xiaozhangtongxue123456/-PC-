<template>
	<div class="header">
		...
		<!-- 列表 -->
		<ul class="lists">
			<li v-for="item in categoryDate.data" :key="item.id" @click="saveBreadcub(item)" :class="{ action: categoryDate.action === item.id }">
				<router-link :to="{ path: '/home/fristcate', query: { fristId: item.id } }">{{ item.name }}</router-link>
			</li>
		</ul>
		<!-- 搜索框 -->
